...
void ecan1WriteTxMsgBufData(unsigned int buf, unsigned int dataLength, unsigned int data1, unsigned int data2, unsigned int data3, unsigned int data4)
{
	...
	// Inverting the byte because in CAN data are BIG ENDIAN - THIS IS LITTLE ENDIAN for MM compatibility
	ecan1msgBuf[buf][3] = ((data1 & 0x00FF) <<8 ) | ((data1 & 0xFF00) >>8 );
	ecan1msgBuf[buf][4] = ((data2 & 0x00FF) <<8 ) | ((data2 & 0xFF00) >>8 );
	ecan1msgBuf[buf][5] = ((data3 & 0x00FF) <<8 ) | ((data3 & 0xFF00) >>8 );
	ecan1msgBuf[buf][6] = ((data4 & 0x00FF) <<8 ) | ((data4 & 0xFF00) >>8 );

}

void ecan1WriteTxMsgBufData_Bit(unsigned int buf, unsigned int dataLength, unsigned int data1, unsigned int data2, unsigned  int data3, unsigned  int data4, unsigned  int data5, unsigned  int data6, unsigned  int data7, unsigned  int data8)
{
...
void clearRxFlags1(unsigned char buffer_number)
{

	if((C1RXFUL1bits.RXFUL2) && (buffer_number==2))
		/* clear flag */
